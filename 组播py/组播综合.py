...
file_contents = []
file_paths = ["北京联通.txt","山西联通.txt","内蒙古电信.txt","甘肃电信.txt","辽宁联通.txt","黑龙江联通.txt","天津联通.txt","江苏电信.txt","陕西电信.txt","四川电信.txt","湖南电信.txt","河南电信.txt","河南联通.txt"]  # 替换为实际的文件路径列表
...
#从整理好的文本中按类别进行特定关键词提取#############################################################################################
keywords = ['北京IPTV', "电视指南", "兵器科技", "世界地理", "文化精品", "风云剧场", "风云音乐", "怀旧剧场", "第一剧场", "女性时尚", "风云足球", "央视台球", "央视高网", 'kk', 'kk', 'kk', 'kk', 'kk', 'kk', 'kk']  # 需要提取的关键字列表
pattern = '|'.join(keywords)  # 创建正则表达式模式，匹配任意一个关键字
#pattern = r"^(.*?),(?!#genre#)(.*?)$" #以分类直接复制
...
###############################################################################################################################################################################
keywords = ['凤凰','凤凰香港','CHC']  # 需要提取的关键字列表
pattern = '|'.join(keywords)  # 创建正则表达式模式，匹配任意一个关键字
#pattern = r"^(.*?),(?!#genre#)(.*?)$" #以分类直接复制
...
###############################################################################################################################################################################
keywords = ['江苏','四川','甘肃','G','河南','山西','天津','4K影视','辽宁','黑龙江','陕西','湖南','湘西','张家界','爱','西部电影','SiTV','峨眉电影']  # 需要提取的关键字列表
pattern = '|'.join(keywords)  # 创建正则表达式模式，匹配任意一个关键字
#pattern = r"^(.*?),(?!#genre#)(.*?)$" #以分类直接复制
...
################
keywords = ['内蒙古','呼市','包头','巴彦淖尔','乌拉特前旗',]  # 需要提取的关键字列表
pattern = '|'.join(keywords)  # 创建正则表达式模式，匹配任意一个关键字
#pattern = r"^(.*?),(?!#genre#)(.*?)$" #以分类直接复制
...
#  获取远程直播源文件
url = "https://raw.githubusercontent.com/lianxiang118/IPTV/main/json/zhibo.txt"          #源采集地址
r = requests.get(url)
